...
Baseline: `459c220`. Current development version: `2.3.0`.

The implementation preserves fixed configured rates, same-IP aggregation, and
independent pacing for different peer addresses. The 2.3 work completes the
remaining structural items from the original performance review that can be
implemented and validated without high-core hardware.
...
| Peer lifecycle | Last reference removes a peer from its rule-local table and retires it with `call_rcu`. Rule flush unlinks all rules, waits one RCU grace period, then releases groups. Per-CPU counters are destroyed from a reclaim workqueue. |
...
| Rule lookup | IPv4 `/32` and IPv6 `/128` rules live only in exact RCU hash indexes, `/0` rules use direct RCU pointers, and only intermediate CIDRs remain in the longest-prefix list. Large sets of host exceptions therefore do not lengthen the default-rule scan. |
| Peer monitoring | `/proc/net/tcp_brutal/peers` walks only rules in the current namespace and then each rule-local peer table. It does not allocate a whole-table snapshot or filter a module-wide peer table. `brutalctl peers` retains `--rule`, `--ip`, `--family`, and `--limit`. |
...
| CAS virtual clock | Do not replace the pacer spinlock without >2 CPU contention measurements. Correct reservation compensation and fairness are more important than a speculative lock removal. |
...
| Generic Netlink | Deferred until there is a real large-dump/event consumer. The namespace-aware proc ABI and bounded CLI remain the supported control/monitoring path. |
...
## Runtime validation available for the two-CPU host

The repository contains the following root-only tests for the available
2-vCPU environment:
...
- `tests/app-group-netns.sh`: two namespaces use the same application group ID
  ...
  the other's group.
- `tests/peer-churn.sh`: creates many distinct logical client source addresses
  without thousands of interface aliases, repeatedly creates and retires
  per-IP groups, then checks live-peer and failure counters. Set `PEERS=10000`
  for the report-scale churn run.
- `tests/benchmark.sh`: repeated same-IP multi-stream throughput measurements.
- `tests/two-cpu-validation.sh`: runs the above correctness, isolation, churn,
  and benchmark sequence with conservative defaults suitable for a 2-vCPU VPS.

Recommended full local run:

```bash
sudo PEERS=10000 RUNS=5 SECONDS_PER_RUN=60 STREAMS=16 RATE_MBPS=200 \
  tests/two-cpu-validation.sh
```

The new runtime tests are intentionally not claimed as executed by GitHub
hosted CI because that environment does not load this out-of-tree module into
its host kernel. Record the VPS results under `benchmark-results/` after the
module is installed and loaded there.

## Existing ARM64 baseline measurements

On the previous Ubuntu 24.04 ARM64 two-CPU host (Neoverse-N1), five 60-second
runs with 16 same-IP streams and a 200 Mbps target produced:

| Build | Minimum | Median | Maximum |
| --- | ---: | ---: | ---: |
| `459c220` | 199.750 Mbps | 199.787 Mbps | 199.855 Mbps |
| 2.2 candidate | 199.718 Mbps | 199.804 Mbps | 199.820 Mbps |

The 2.2 candidate median differed by +0.008%, within run-to-run variation, and
configured-rate error was below 0.15%. These measurements predate the 2.3
structural split and must not be presented as 2.3 benchmark results.
...
## Release acceptance for 2.3

Before tagging 2.3.0 on the available host:

1. All GitHub compile, userspace, and formatting jobs must be green.
2. Run `tests/two-cpu-validation.sh` with `PEERS=10000` and five 60-second
   benchmark repetitions.
3. Confirm zero peer allocation/insertion failures and zero unexpected fallback
   connections during the normal-memory run.
4. Compare throughput and CPU use with the saved 2.2 build; configured-rate
   accuracy should remain within 2%, preferably within 1%.
5. Save the resulting benchmark JSON and the output of
   `/proc/net/tcp_brutal/stats` with the release evidence.
...
service. Keep an installed known-good DKMS build as the persistent rollback
target while validating 2.3.
